...
import 'config.dart'; // Import the config
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> signUp() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('${Config.apiBaseUrl}/signup'),
        body: jsonEncode({
          'username': usernameController.text,
          'password': passwordController.text,
        }),
...
class _MyHomePageState extends State<MyHomePage> {
    ...
    try {
      if (areFiltersApplied()) {
        ...
        if (isLoggedIn) {
          ...
          final uri = Uri.http(Config.apiBaseUrl.replaceAll(RegExp(r'https?://'), ''), '/filter', queryParams);
          print(uri);
          ...
          print("Sent JSON data: ${jsonEncode(queryParams)}");
          final uri =
              Uri.http(Config.apiBaseUrl.replaceAll(RegExp(r'https?://'), ''), '/filter/notlogin', queryParams);
          print(uri);
          final response = await http.get(uri);
          if (response.statusCode == 200) {
            Iterable l = json.decode(response.body);
        ...
        final response =
            await http.get(Uri.parse('${Config.apiBaseUrl}/contacts'));
        if (response.statusCode == 200) {
          Iterable l = json.decode(response.body);
          contacts = List<Contact>.from(
              l.map((model) => Contact.fromJson(model)).toList());
  ...
  Future<void> _deleteContact(int id) async {
    final response =
        await http.delete(Uri.parse('${Config.apiBaseUrl}/contacts/$id'));
    if (response.statusCode == 200) {
      _fetchContacts();
    } else {
